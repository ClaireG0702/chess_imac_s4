    ...
    if (!pieces.empty()) {
        ...
        if (piece) {
            ...
            if (!possibleMoves.empty()) {
                ...
                int dirRow = (moveRow == row) ? 0 : (moveRow > row) ? 1 : -1;
                int dirCol = (moveCol == col) ? 0 : (moveCol > col) ? 1 : -1;
                ...
                if (incorrectDistance > 0) {
                    ...
                    if (m_board.isValidPosition(incorrectRow, incorrectCol) && 
                        m_board.getPieceAt(incorrectRow, incorrectCol) == nullptr) {
            ...
            if (possibleMoves.size() >= 2) {
                ...
                possibleMoves.erase(
                    std::remove_if(possibleMoves.begin(), possibleMoves.end(),
                        [row, col](const std::pair<int, int>& move) {
...
void GameState::applyCharacteristicEvents(Piece* movedPiece, int toRow, int toCol) {
    ...
    if (movedPiece->hasCharacteristic(PieceCharacteristic::Dyscalculia)) {
        ...
        if (std::abs(error) > 0.5) {  // If error magnitude is significant
            applyDyscalculiaEvent();
        }
    }
    ...
    if (movedPiece->hasCharacteristic(PieceCharacteristic::Genderfluid)) {
        double changeTime = m_pieceEventManager->getGenderfluidChangeTime();
        if (changeTime > 0.5) {  // If time value exceeds threshold
            applyGenderfluidEvent();
        }
    }
    ...
    if (movedPiece->hasCharacteristic(PieceCharacteristic::ADHD)) {
        double chaotic = m_pieceEventManager->getADHDRandomBehavior();
        if (std::abs(chaotic) > 1.0) {  // If chaotic value is high (Cauchy heavy tails)
            applyADHDEvent();
        }
    }
    ...
    if (movedPiece->hasCharacteristic(PieceCharacteristic::Autism)) {
        ...
        if (isolatedCount > 10) {  // If isolation count exceeds threshold
            applyAutismEvent();
        }
    }
}
